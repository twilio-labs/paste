...
A Badge can also be used to trigger an in-page action, such as showing a [Popover](/components/popover) or [Modal Dialog](/components/modal).
...
Use small Badges only when vertical density is a concern. The guidelines for using variants in a small Badge are the same as in their default size.
