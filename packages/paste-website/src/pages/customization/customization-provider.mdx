...
Similar to the Paste [ThemeProvider](/tokens/theme-package), the `CustomizationProvider` uses [React Context](https://reactjs.org/docs/context.html) to provide a [theme object](/tokens/theme-package/#theme) to any descendant components in the tree.
...
Custom themes can also be imported as a json object, along with the `CustomizationProvider`. Remember, the custom theme must follow the format of the Paste theme object.

After importing the custom theme needs to be passed to the `theme` prop on the `CustomizationProvider`. The `CustomizationProvider` should wrap the root of your application, or part of the application you would like to theme.
