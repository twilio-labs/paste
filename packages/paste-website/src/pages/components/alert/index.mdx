...
<ComponentHeader
  ...
  githubUrl="https://github.com/twilio-labs/paste/tree/master/packages/paste-core/components/alert"
  storybookUrl="https://twilio-labs.github.io/paste/?path=/story/components-alert"
...
<LivePreview scope={{Alert, Text}} language="jsx">
  ...
  <Text as="span">
    <strong>Whoa there!!</strong> Hot alert based content below. Proceed with excitement.
  </Text>
</Alert>`}
</LivePreview>
...
Don't add a close button to the alert if all these conditions are met:

- The alert communicates a persistent status of the system or the account
- The information is relevant to anything the user would be trying to do wherever the alert is present.
- The information isn't otherwise accessible to the user.

Keep in mind that the longer an alert is present, the more it starts looking like part of the page and loses its interruptive intent. This may make users start to ignore important alert information.
...
Keep alert text brief by placing only the highest priority information in an alert. Too much text in an alert can overwhelm a user. A good rule of thumb is keeping the alert text to a single line in a desktop-sized container, or limiting it to 90 characters.
...
- **Body text.** Use the default text style. Provide specificity on the issue and a resolution if applicable via an [anchor](/components/anchor). Be cautious about using a button in an alert since it may compete with other buttons on the screen and should generally be used only to trigger an action. If you must use a button, use a [small button](/components/button/#small-button).
...
<DoDont>
  <Do title="Do" body="Use only 1 alert per page. Use 2 alerts only if absolutely necessary and if the alerts are placed in different containers.">
    <Alert variant="error">
      <Text as="span">Your account balance is low and your credit card has expired. As a result we have suspended your account</Text>
    </Alert>
  </Do>
...
<DoDont>
  <Do title="Do" body="Place global alerts at the top of the page. Place all other alerts at the top of the container it relates to beneath the heading.">
      <Box padding="space40">
        <Card>
          <Heading as="h2" variant="heading20">Section Heading</Heading>
          <Box marginBottom="space70">
  ...
  <Dont title="Don't" body="Don't cover other UI elements with an alert.">
    <Box padding="space40" position="relative">
      <Heading as="h2" variant="heading10">Billing information</Heading>
...
<DoDont>
  <Do title="Do" body="Use anchors to link to more information that may be helpful to users to resolve or get more clarity around an alert message." >
    <Alert variant="neutral">
      <Text as="span">A call to action <Anchor href="#">Click here</Anchor></Text>
    </Alert>
  </Do>
  <Dont title="Don't" body="Avoid using buttons in alerts unless it's used to trigger an action.">
    <Alert variant="neutral">
      <Text as="span">
        A call to action <Button size="small" variant="primary">Click here</Button>
      </Text>
    </Alert>
  </Dont>
</DoDont>
...
<DoDont>
  ...
  <Dont title="Don't" body="Don't use alerts to communicate every change in a product (e.g., marketing messages, trial status) so that important messaging isn't diluted." center />
</DoDont>
...
| ---------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- |
| background-color | <ul><li>Error: color-background-error-lightest</li><li>Neutral: color-background-neutral-lightest</li><li>Warning: color-background-warning-lightest</li></ul>                                        | No          |
