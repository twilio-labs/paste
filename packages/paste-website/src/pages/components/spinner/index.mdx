...
<ComponentHeader
  ...
  githubUrl="https://github.com/twilio-labs/paste/tree/master/packages/paste-core/components/spinner"
  storybookUrl="https://twilio-labs.github.io/paste/?path=/story/components-spinner--default"
...
The spinner is an icon component that can be used in situations where you would like to communicate a loading state to the user. It inherits a lot of properties from the [Icon component](/icon-system/).
...
The only requirement is that you set whether it is a decorative Spinner or not, which follows the [same guidelines as icons](/icon-system/#accessibility).

<Callout>
  <CalloutTitle>Accessibility</CalloutTitle>
  <CalloutText>
    Please pay careful attention to our{' '}
    <SiteLink to="/icon-system/#accessibility">icon accessibility guidelines</SiteLink>.
  </CalloutText>
</Callout>
...
The Spinner, like any other icon, can be re-colored to meet your needs. Set an `iconColor` in the [same way you would with an icon](/icon-system/#adapting-the-icons-color).
...
>
  {`<Spinner iconColor="colorTextSuccess" decorative={false} title="Loading" />`}
</LivePreview>
