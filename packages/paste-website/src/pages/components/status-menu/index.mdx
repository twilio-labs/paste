export const meta = {
  ...
  description:
    'A Status Menu should be used to communicate the status of a process, or connection to an application or service and provide a way to update that status type.',
...
The Status Menu can be used when you want to both display and change the status of a connection or process. It closely follows the [Status Pattern](/patterns/status) and comes with preconfigured options for displaying the status of both processes and connections, using the correct icons for each.

### Accessibility

The Status Menu implements a Menu and supports all the same keyboard navigation that the [Menu](/components/menu) component does.
...
The Status Menu consists of `StatusMenu`, `StatusMenuBadge`, `StatusMenuItem`, `StatusMenuItemRadio`, `StatusMenuItemCheckbox`, and `useStatusMenuState`. These are all name sakes of their [Menu](/components/menu) equivilants. It also comes with one additional component called `StatusMenuItemChild`.

### Status Menu Item Child

The `StatusMenuItemChild` is where the status pattern is implemented. It takes a `variant` prop that be set to a value that directly relates to one of the connection or process status types. It pairs the correct icon, with the color implementation and text of your choice.
