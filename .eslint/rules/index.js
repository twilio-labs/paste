...
const allRuleFiles = fs.readdirSync('.eslint/rules').filter((file) => file !== 'index.js' && !file.endsWith('test.js'));
...
const rules = Object.fromEntries(allRuleFiles.map((file) => [path.basename(file, '.js'), require('./' + file)]));
