...
import { basicExample, disabledExample, pressedExample } from "../../../component-examples/KeyboardKeyExamples";
...
Use Keyboard Keys for keyboard commands or shortcuts. When showing a key combination, do not put a “+” between Keyboard Keys.
...
Pressed Keyboard Keys can be used to give visual feedback when a key is pressed.
...
The `KeyboardKeyGroup` accepts state returned from the [`useKeyCombination`](#useKeyCombination) hook that allows pressedStyling to be enabled. You must also specify `keyText` on the component to detect the correct key pressed.

<LivePreview scope={{ KeyboardKeyGroup, KeyboardKey, useKeyCombination }} noInline language="jsx">
  {pressedExample}
</LivePreview>
...
Any modifier keys + any of <KeyboardKey>Tab</KeyboardKey>, <KeyboardKey>Enter</KeyboardKey>, <KeyboardKey>Space</KeyboardKey>, or <KeyboardKey>Escape</KeyboardKey>.

<List marginBottom="space30">
  <ListItem>
