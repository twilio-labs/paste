...
export default () => {
  ...
  fail(
    `You have marked the following Paste package(s) as a MAJOR update, but have not provided an upgrade guide. Please update https://paste.twilio.design/core/upgrade-guide to inform our users what to do about this Major update to Paste.
    ${changesetsWithMajorUpdate.join(', ')}`
  );
};
