...
<Callout variant="neutral" marginY="space70">
  <CalloutHeading as="h4">
    Does your product have a use case where there are only one or two statuses on a page?
  </CalloutHeading>
  <CalloutText>
...
For convience and consistency, we have created a [`StatusBadge`](/components/badge#status-badge) component that can be used to compose a status icon with a text label.
