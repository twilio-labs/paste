...
A custom theme can be created either manually or using the [Paste Theme Designer](https://paste-theme-designer.netlify.app/edit). A [Paste Theme](/core/theme-package) is categorized into groups of [Design Tokens](/tokens). The value of any Design Token from any Group can be changed when supplied to the Customization Provider via the theme object. The following token groups are available for customization:
