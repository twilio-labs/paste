...
2. Clone your newly forked Paste repository:
```
git clone <url>
```
3. Navigate inside the newly cloned repository with `cd paste`
4. Move any number of `svg` icon files into the `packages/paste-icons/svg/` folder.
5. From the root Paste folder, run this automatic conversion command:
```
yarn workspace @twilio-paste/icons convert-new
```
6. Verify the `packages/twilio-icons/src` folder to see if your new icons were added correctly.
7. At this point, run the following command to update our documentation with the new icons:
```
