...
export const tokenTemplate = ({name}: {name: string}): string =>
  `export const ${camelCase(name)} = "var(--${kebabCase(name)})";`;
...
export const es6CustomPropertiesTokenFormat = (result: ImmutableStyleMap): string => {
