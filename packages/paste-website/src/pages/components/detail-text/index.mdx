...
export const pageQuery = graphql`
  {
    allPasteComponent(filter: {name: {eq: "@twilio-paste/detail-text"}}) {
      edges {
        node {
          name
          description
    ...
    mdx(frontmatter: {slug: {eq: "/components/detail-text/"}}) {
      fileAbsolutePath
      frontmatter {
        slug
        title
    ...
    allAirtable(filter: {data: {Feature: {eq: "Detail Text"}}}) {
      edges {
        node {
          data {
            Documentation
...
<NormalizedComponentHeader
  ...
  storybookUrl="/?path=/story/components-detail-text"
  data={props.data}
/>
...
Caption Text is available in the Paste Components Figma library.
