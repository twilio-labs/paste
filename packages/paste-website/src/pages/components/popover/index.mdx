...
To internationalize the popover, simply pass different text as children to the popover's contents and the `aria-label` prop. The only exception is the dismiss button–to change the button’s text, use the `i18nDismissLabel` prop.
