...
Use 'ProgressStepContent' to add additional content to a `orientation="vertical"`, `as="div"` Progress Step.

Use additional content when each step requires minimal content or actions—like opening a modal, redirecting, or entering text, and users need to complete steps in a specific sequence, with progress visibly tracked throughout.
