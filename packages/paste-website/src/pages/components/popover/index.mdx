...
### Modifying the Popover Button

The PopoverButton by default renders a Button component and accepts all of its props, which are listed [on the Button page](/components/button#props).
...
To make the PopoverButton a Badge component, pass the prop `as="badge"`. Then the PopoverButton accepts all of the badges props and badge variants, which are listed [on the Badge page](/components/badge#props).
...
See the [button](/components/button) and [badge](/components/badge) components.
...
Defaults to `'button'`. Determines if a [button](/components/button) or [badge](/components/badge) is rendered.
