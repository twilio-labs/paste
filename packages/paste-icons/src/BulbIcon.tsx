...
export interface BulbIconProps extends IconWrapperProps {
  title?: string;
  decorative: boolean;
}

const BulbIcon = React.forwardRef<HTMLElement, BulbIconProps>(
  ({as, display, element = 'ICON', size, color, title, decorative}, ref) => {
    const titleId = `BulbIcon-${useUID()}`;

    if (!decorative && title == null) {
      throw new Error('[BulbIcon]: Missing a title for non-decorative icon.');
    }
