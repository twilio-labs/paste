...
Text to explain what inputs are accepted, like minimum or maximum values or step intervals, especially if there's no default value.
Help Text should also include any important contextual information about what the number input represents.

<LivePreview scope={{Input, Label, HelpText}} language="jsx">
  {`<>
  <Label htmlFor="reload_amount">Number Input</Label>
...
To internationalize Number Input, provide translated strings to be used for the increment and decrement button accessible labels. Use the `i18nStepUpLabel` and `i18nStepDownLabel` props.
...
A Help Text inline error is placed below the field to inform a user of any errors in their value. Change the Help Text to `variant=“error”` and add error copy before the original help text copy.
