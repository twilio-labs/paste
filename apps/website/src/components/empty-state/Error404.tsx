...
const Error404 = ({pathname, componentList, primitiveList}: Error404Props): React.ReactNode => {
  const pathParts = pathname.split('/');
  const pageName = pathParts[pathParts.length - 1];
  const packageName = `@twilio-paste/${pageName}`;
  const packageObj = [...componentList, ...primitiveList].find(({name}) => name === packageName);
