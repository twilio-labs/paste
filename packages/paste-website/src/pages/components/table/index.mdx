...
<NormalizedComponentHeader
  ...
  storybookUrl="/?path=/story/components-table--"
  data={props.data}
/>
...
- Any component can be placed in a Table, but tables should only contain static textual and numeric data, rather
  ...
  selecting rows, or deleting rows), use <SiteLink href="/components/data-grid">Data Grid</SiteLink> instead.

<Callout variant="neutral" marginY="space70">
  <CalloutHeading as="h4">Accessibility insight: navigating tables with a screen reader</CalloutHeading>
  <CalloutText>
...
Tables support a "sticky header", which can be useful in tables with a large amount of data. A sticky header will allow
a user to keep the context of what each column contains as they scroll through the data contained in the table.

Set the boolean property `stickyHeader` on the `THead` component, to enable the sticky header.

<LivePreview scope={{Table, THead, TBody, TFoot, Tr, Th, Td, Text, Box}} language="jsx">
...
<LivePreview scope={{Table, THead, TBody, Tr, Th, Td, Text, DetailText}} language="jsx">
  {`<Table>
  <THead>
    <Tr>
      <Th>User</Th>
...
**When a single row contains an error,** do not explain the full error within the row. Use an indicator, like an icon or short message. Then use other methods, like selectable rows or popups, to communicate complete error and resolution information. 

**When the same error affects multiple rows,** consider a consistent indicator for each affected row, like an icon. Then use a [Callout](/components/callout) at the page level to explain the error and resolution.
...
All the valid HTML attributes for the `thead` element are supported including the following:
