...
export interface ListAddIconProps extends IconWrapperProps {
  title?: string;
  decorative: boolean;
}

const ListAddIcon = React.forwardRef<HTMLElement, ListAddIconProps>(
  ({as, display, element = 'ICON', size, color, title, decorative}, ref) => {
    const titleId = `ListAddIcon-${useUID()}`;

    if (!decorative && title == null) {
      throw new Error('[ListAddIcon]: Missing a title for non-decorative icon.');
    }
