...
If you choose to not use the design primitive components directly (and, if so, we'd love to learn more about your use case), check out the options provided by [Theme Package](theme-package). This package showcases ways to harness design tokens in React in a few different ways.

### Not using React?

If you aren't using React, hop on over to the [Design Tokens Package](token-package). This package distributes the tokens and their values via NPM in a variety of different formats, for a bunch of different technologies. Also, if you're doing this, reach out at <em>#help-design-system</em> - we're really curious about what you're up to!
