...
- Switch don't natively support the `required` attribute. In cases where a Switch is required to be checked:
...
- When in an error state display an inline error message below the offending Switch that clearly describes the error along with an icon that depicts the severity.
