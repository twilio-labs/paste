...
description: Display heading is large, expressive text that stands out from traditional headings and is used for marketing and attention grabbing purposes.
...
export const pageQuery = `
  {
    allPasteComponent(filter: {name: {eq: "@twilio-paste/display-heading"}}) {
      edges {
        node {
          name
          description
    ...
    mdx(frontmatter: {slug: {eq: "/components/display-heading/"}}) {
      fileAbsolutePath
      frontmatter {
        slug
        title
    ...
    allAirtable(filter: {data: {Feature: {eq: "Display Heading"}}}) {
      edges {
        node {
          data {
            Documentation
