...
When referring to `ChatComposer` it is the rich text area only. You can use the `ChatComposer` component only, or use it within the `ChatComposerContainer` for consistent styling across chat features.
...
- **ChatComposerActionGroup**: A collection of action buttons and controls, allowing users to perform various actions.
...
Chat composer components suite demonstrating responsive attachments
