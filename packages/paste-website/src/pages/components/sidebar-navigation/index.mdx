...
Sidebar navigation comprises a set of user interface controls, designed to facilitate both flat and hierarchical navigation structures within an application. The navigation components are presentational and can be utilized to create navigation structures ranging from simple lists to complex, deeply nested structures.

We recommend limiting the nesting depth to three levels. For navigation beyond this depth, consider also using the [In-page Navigation](/components/in-page-navigation).

### Accessibility

Nested sidebar navigation employs the [Disclosure pattern](https://www.w3.org/WAI/ARIA/apg/patterns/disclosure/).
...
In its most basic form, the Sidebar Navigation can be utilized to create a straightforward, flat list of navigation items. This is ideal for smaller applications or single-product use cases.
...
Sidebar Navigation also supports the creation of hierarchical information architectures using nested disclosures. You can establish "groups" of navigation sections by deploying the `SidebarNavigationSeparator` between groups of items.
