...
Avatar sizing works similarly to [Paste Icon](/components/icon/usage-guidelines#resizing-icons) sizing. Pass any of the icon size tokens (`sizeIcon10`, `sizeIcon20`, `sizeIcon30`, etc.) to Avatar’s `size` prop.
...
Provide the Avatar with an `icon` prop to display an icon. You must import the icon before passing it to the `icon` prop. You **must** still provide a `name` prop. The icon must be a <a href="/components/icon/">Paste Icon</a>.
