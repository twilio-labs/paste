...
Grid is built using the [Flex component](/utilities/flex) and can be made responsive.
...
Use `span` to allow content to span multiple columns, such as a layout with a sidebar that takes up 1/4 of the screen and a content area that takes up 3/4 of the screen.
...
<Callout variant="warning">
  <CalloutTitle as="h4" variant="heading40">An Important Note About Span and Offset</CalloutTitle>
  <CalloutText>When using the <strong>span</strong> or <strong>offset</strong> props, base your layout off a 12-column grid. For example, if you want to have two columns with one spanning 8 columns, one column would span 8 and the other would need to span 4.</CalloutText>
</Callout>
...
#### Stacked columns in a responsive view

Use `vertical` to stack columns vertically instead of horizontally. Use this when your layout needs to respond to a smaller view.
