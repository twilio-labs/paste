...
<NormalizedComponentHeader
  ...
  storybookUrl="/?path=/story/components-select--"
  data={props.data}
/>
...
At its most basic, a Select has an options list that’s styled according to the browser default. A [Combobox](/components/combobox) has a Twilio-styled options list and can allow for additional functionality like typeahead.

Use a Select when:

- You need a native picker experience, especially on mobile devices.
- Users will be selecting from a list of 4-10 options, or a sorted list of highly familiar options, like a alphabetical list of states or countries.
- You need the component to work out-of-the-box across all operating systems and browsers.

Use a [Combobox](/components/combobox) when:
...
If a user is able to choose an invalid option, change a Select to its error state and use [Help Text](/components/help-text) to add an inline error. The Help Text inline error is placed below the field to inform a user of any errors in their value. If help text exists, the error text should replace and repeat the help text.
...
<DoDont>
  <Do
    center
    title="Do"
    body="Keep option text concise and simple. If choosing options require more explanation, expand in help text or consider using a Radio Group instead."
  >
    <Box width="100%" padding="space60">
