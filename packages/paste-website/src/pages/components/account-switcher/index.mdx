export const meta = {
  ...
  description: 'An account switcher is a stylized menu badge with a list of actions related to a users accounts.',
...
The Account Switcher is designed to be used anywhere a user needs to be able to switch between accounts or sub-accounts, and can usually be found in the Topbar.

The Account Switcher is a Menu Badge, with groups of account options displayed in the menu dropdown. These include recent accounts, account settings and account view controls. The Menu Badge itself should display the current account the user is viewing and update if a new account is selected.

The Account Switcher is an implementation of the [Menu](/components/menu) component and shares a lot of sub components and hooks. That means it share a lot of the API.
...
Container for the Account Switcher Menu.

### Account Switcher Group

Used to group similar items together in the Account Switcher Menu. An example of this might be a list of recent accounts with in the Menu.
...
A menu item that can perform a single selection of an item with in a named group. Similar to a radio button group, only one item can be selected at a time. Each item in the group should have a `name` and `value` and must be contained in a Group.

### Account Switcher Separator

Simple horizontal rule used to seperate groups or individual items.

### useAccountSwitcherState state hook

React Hook used to initialise the Account Switcher with vaious options. It returns state and actions that can be taken on the Account Switcher menu.
