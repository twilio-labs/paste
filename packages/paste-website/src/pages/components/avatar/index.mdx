...
**Note:** Avatar can only display _one_ of the following options (initials, image, icon) at a time. If both `src` and `icon` props are provided in addition to `name`, the Avatar will display the icon, not the image.
