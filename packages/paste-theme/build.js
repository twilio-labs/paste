...
// eslint-disable-next-line @typescript-eslint/explicit-function-return-type
async function build(packageJson, optionalPlugins = []) {
  ...
  console.log("Building", outFileCJS, outFileESM, ...optionalPlugins);
  ...
  await esbuild
    .build({
      ...
      plugins: [esbuildPluginVersionInjector(versionInjectorConfig), ...optionalPlugins],
    })
    .catch((error) => {
      console.error(error);
