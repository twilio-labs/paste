...
description: An accessible modal dialog that sits on the right side of the page.
...
export const pageQuery = graphql`
  {
    allPasteComponent(filter: {name: {eq: "@twilio-paste/side-modal"}}) {
      edges {
        node {
          name
          description
    ...
    mdx(frontmatter: {slug: {eq: "/components/side-modal/"}}) {
      fileAbsolutePath
      frontmatter {
        slug
        title
    ...
    allAirtable(filter: {data: {Feature: {eq: "Side Modal"}}}) {
      edges {
        node {
          data {
            Documentation
...
| `justify?`  | `'flex-start', 'flex-end', 'space-around', 'space-between', 'space-evenly', 'stretch', 'inherit', 'initial', 'revert', 'unset',` | Sets the `justify-content` CSS property. Accepts all valid options.                       | `'flex-end'`                  |
