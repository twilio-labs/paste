...
Menu items come with the ability to be selected, in the same way as a checkbox or radio button. This is useful when a user is asked to perform a persistant action, where the selection should be preserved for future reference. Selections can either be single, or multiple, just like radio and checkboxes.
...
Status is represented via the `MenuBadgeStatus` and `MenuItemStatus` components. `MenuBadgeStatus` is a wrapper around the [`StatusBadge`](/components/badge#status-badge) that accepts both `MenuButton` props and the `StatusBadge` variant prop. The `MenuItemStatus` component is a composed child of `MenuItem`, that also provides a collection of variants that map to the [Status Pattern](/patterns/status). It can be used in conjunction with `MenuItem`, `MenuItemCheckbox` and `MenuItemRadio`.
...
<Callout variant="neutral" marginY="space70">
  <CalloutHeading as="h4">Note</CalloutHeading>
  <CalloutText>
...
| element? | `string`              | Overrides the default element name to apply unique styles with the Customization Provider                                                                                                                                                          | 'MENU_BADGE_STATUS' |
