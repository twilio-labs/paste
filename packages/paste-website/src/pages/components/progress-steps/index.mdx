...
- Each step icon has a `title` prop whose value is the current status of that step (e.g. complete, incomplete, etc.) to inform screen readers of a step's status. This label is adjustable for i18n.
...
user navigates to the step that has errors, set the status of the step to `current`
and use in-page validation to communicate the errors in the current step.
Check out the [Error state pattern](https://paste.twilio.design/patterns/error-state) for more guidance.

## Composition Notes

- Each step must have a label. Keep the label concise with fewer than 5 words per label.
- Consider skipping articles like “a” or “the” to shorten the step.
...
<DoDont>
  <Do center title="Do" body="Use Progress Steps for complex, noncontinuous tasks with 3–5 steps">
