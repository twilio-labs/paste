...
Tooltips can only contain text content, and should contain only information that is useful additional context or guidance for the user. If you need to include an [Anchor](/components/anchor) or icons, use a [Popover](/components/popover) instead.

Never put information essential to task completion in a Tooltip. Furthermore, Tooltips are easily overlooked and disappear when not focused on, so consider whether the information may be better served by another component. Use [Help Text](/components/help-text), which is persistent and more obvious, for essential information.
...
Use full sentences and punctuation. Include no more than 3 lines of text.
...
<DoDont>
  ...
  <Dont
    title="Don't"
    body="Don't use Tooltips on non-focusable elements, like icons and disabled buttons."
    preview={false}
  />
</DoDont>
