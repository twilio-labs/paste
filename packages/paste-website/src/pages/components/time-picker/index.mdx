...
{/_ TODO: before merging, update "has documentation" in airtable _/}
...
{/_ TODO: update screenshots and details _/}
    ...
    <TBody>
      <Tr verticalAlign="middle">
        <Td textAlign="left">
          ...
          <UnorderedList>
            ...
            <ListItem>"Today" button selects current time and closes calendar pop-up</ListItem>
          </UnorderedList>
        </Td>
        <Td textAlign="right">
          <Box
      ...
      <Tr verticalAlign="middle">
        <Td textAlign="left">
          ...
          <UnorderedList>
            <ListItem>Default value is the current time</ListItem>
            <ListItem>
              Clicking on the input field triggers the calendar pop-up and places focus on the value (calendar icon not
              present)
            </ListItem>
            <ListItem>
              Circular button (in between navigation left/right arrows) navigates calendar pop-up back to selected time
            </ListItem>
          </UnorderedList>
        </Td>
        <Td>
      ...
      <Tr verticalAlign="middle">
        <Td textAlign="left">
          ...
          <UnorderedList>
            ...
            <ListItem>No functionality to navigate user back to current or selected time</ListItem>
          </UnorderedList>
        </Td>
        <Td>
          <Box
...
. The parsed <inlineCode>value</inlineCode>, however, is always returned in <inlineCode>HH:mm</inlineCode> format (using
...
The function accepts two parameters: (1) the time value that should be formatted (e.g. <inlineCode>'2021-03-24'</inlineCode>), and (2) the desired time format (e.g. <inlineCode>'MM dd yyyy'</inlineCode>). It uses a library called <Anchor href="https://github.com/time-fns/time-fns" showExternal>time-fns</Anchor> to return the given time in the desired format. Time format must follow the <Anchor href="https://time-fns.org/v2.21.3/docs/format" showExternal>tokens outlined by time-fns</Anchor>.
...
When using multiple Time Pickers, stack them vertically with `$space-80` spacing between each field. To stack them, you can use either a <Anchor href="/primitives/box">Box</Anchor> or a <Anchor href="/layout/stack">Stack</Anchor> (as seen in the example below).

<LivePreview scope={{TimePicker, Label, HelpText, useUID, Anchor, Box, Stack}} noInline>
  {StackOfPickers}
</LivePreview>

Avoid placing multiple pickers on the same horizontal row to help make it easier to scan a page vertically. Use a <Anchor href="/layout/grid">Grid</Anchor> if you need to place them horizontally.
...
- Use help text to provide instruction if needed. For example, don't use "Enter the time you wish to receive your bill below" as label text. Instead, use "Billing time" as a label and "Your account will be automatically billed on the above time." as help text.
...
Valitime Time Picker fields on form submission.
...
Usage of the <inlineCode>min</inlineCode> and <inlineCode>max</inlineCode> props on Time Picker is encouraged if certain times are invalid.
...
<Callout>
  ...
  <CalloutText>
    Because Time Picker has a default display value of <inlineCode>mm/dd/yyyy</inlineCode> (and defaults to the current
    time on Safari), any value that gets passed into <inlineCode>placeholder</inlineCode> will be overwritten (and
    effectively ignored).
  </CalloutText>
</Callout>
  ...
  return (
    <>
      ...
      <TimePicker
        ...
        id="foo"
        name="foo"
        onChange={(evt) => formatReturnTime(evt.target.value, 'MM/dd/yyyy')}
        required
      />
...
| Prop       | Type   | Description                                                                                                           | Default |
| ---------- | ------ | --------------------------------------------------------------------------------------------------------------------- | ------- |
| timeValue  | string | Time value to be formatted. Must be in <inlineCode>yyyy-mm-dd</inlineCode> format.                                    | null    |
| timeFormat | string | Speficy the format in which to return the timeValue. Formats come from the <inlineCode>time-fns</inlineCode> library. | null    |
...
| htmlFor       | string               | Sets the for of the label. Should match the id of `TimePicker`. Required. | null      |
| disabled?     | boolean              | Shows the time picker is disabled.                                        | false     |
| required?     | boolean              | Shows the time picker is required.                                        | false     |
