...
<NormalizedComponentHeader
  ...
  storybookUrl="/?path=/story/components-modal--"
  data={props.data}
/>
...
If you need to to guarantee a user acknowledges a critical piece of information, use an [Alert Dialog](/components/alert-dialog) instead.
...
<Callout variant="neutral" marginY="space70">
  <CalloutHeading as="h4">Provide an accessible label</CalloutHeading>
  <CalloutText>
    Be sure to assign the <inlineCode>aria-labelledby</inlineCode> property on the modal container to the id of the
    modal heading.
  </CalloutText>
</Callout>
...
-The body should expand on the topic in the header. If you need multiple sections of content with Headings in the body, use a [Heading with the `heading40` variant](/components/heading#heading-as-h-2-with-heading-40-variant).
...
Do not use modals to show important warnings, since they can be dismissed without the user acknowledging the information. Instead, use an [Alert Dialog](/components/alert-dialog) instead.

Do not use modals to show error messages. Refer to the error state pattern for [additional guidance on components](/patterns/error-state#when-to-use-which-component).
