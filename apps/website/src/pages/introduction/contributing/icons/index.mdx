...
5. Add all the new svg icon files into the packages/icons/svg/ folder.
6. From the root Paste folder, run the following command in your terminal: `yarn workspace @twilio-paste/icons convert`
7. Verify your new icons were added correctly by checking the packages/icons/src folder to see the newly created source files.
