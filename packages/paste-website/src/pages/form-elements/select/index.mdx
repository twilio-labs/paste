...
import {FormLabel, FormHelpText, Select, Option, OptionGroup} from '@twilio-paste/form';
...
<ComponentHeader
  ...
  storybookUrl="https://twilio-labs.github.io/paste/?path=/story/components-form--select-options"
...
Select allows a user to make a selection from a list of options in a form field.

By default, it will show the first option in the list. However, you can also set a different pre-selected option from your list to show.

### What's the difference between select and a combobox?
...
<UnorderedList>
  ...
  <ListItem>
    Users will be selecting from a list of 4-10 options, or a sorted list of highly familiar options (e.g., alphabetical
...
<UnorderedList>
  ...
  <ListItem>You need to show more than text in an option (e.g., text paired with an icon).</ListItem>
  <ListItem>You need placeholder text in the field to show an example selection.</ListItem>
  <ListItem>
    Users would benefit from typeahead functionality (e.g., autocomplete, search). For example, typeahead may be useful
    when users need to select from a list of more than 10 options. If you have a much longer list, consider lazy loading
    the list to improve page load performance.
  </ListItem>
</UnorderedList>
...
<LivePreview scope={{Select, Option}} language="jsx">
  {`<>
  <Select id="console_team" htmlFor="console_team">
    <Option value="messaging-console">Messaging Console</Option>
    <Option value="numbers-console">Numbers Console</Option>
    <Option value="vnv-console">VNV Console</Option>
  </Select>
</>`}
</LivePreview>
...
<Callout>
  ...
  <CalloutText>
    Make sure to connect the <inlineCode>FormLabel</inlineCode> to the <inlineCode>Select</inlineCode>. This is done with the <inlineCode>htmlFor</inlineCode> prop on the label, and the <inlineCode>id</inlineCode> prop on the select. Those two need to match.
  </CalloutText>
</Callout>

<LivePreview scope={{Select, Option, FormHelpText, FormLabel}} language="jsx">
  {`<>
  <FormLabel htmlFor="console_team" required>Console team</FormLabel>
  <Select id="console_team" htmlFor="console_team" required>
    <Option value="messaging-console">Messaging Console</Option>
    <Option value="numbers-console">Numbers Console</Option>
    <Option value="vnv-console">VNV Console</Option>
  </Select>
  <FormHelpText variant="default" id="console_team_help_text">Please choose your team.</FormHelpText>
</>`}
</LivePreview>
...
<LivePreview scope={{Select, Option, FormHelpText, FormLabel}} language="jsx">
  {`<>
  <FormLabel htmlFor="console_team">Console team</FormLabel>
  <Select
    id="console_team"
    htmlFor="console_team"
...
You can instead select a <inlineCode>defaultValue</inlineCode> and supply that on render as the <inlineCode>value</inlineCode> property for the select.
...
Use an optgroup to create groupings of options within the select element.
...
<LivePreview scope={{Select, Option, OptionGroup, FormHelpText, FormLabel}} language="jsx">
  {`<>
  <FormLabel htmlFor="configure_trunk" required>Configure with trunk</FormLabel>
  <Select
    id="configure_trunk"
    htmlFor="configure_trunk"
  >
    <OptionGroup label="SIP Trunks">
        <Option value="TK1234567890">TK1234567890</Option>
        <Option value="TK2345678901">TK2345678901</Option>
        <Option value="TK3456789012">TK3456789012</Option>
    </OptionGroup>
    <OptionGroup label="BYOC Trunks">
        <Option value="BY1234567890">BY1234567890</Option>
        <Option value="BY2345678901">BY2345678901</Option>
        <Option value="BY3456789012">BY3456789012</Option>
    </OptionGroup>
  </Select>
  <FormHelpText variant="default" id="configure_trunk_help_text">Select a trunk to configure your phone number.</FormHelpText>
</>`}
</LivePreview>

### Select with multiple values

You can use the multiple attribute on the select component to select multiple options.

- **size** - Attribute on select which will determine how many items are rendered.
- **value** - When multiple attribute is set, this attribute on the select component must be an array.

<Callout>
  <CalloutTitle as="h4">Hot accessibility tip</CalloutTitle>
  <CalloutText>
    Avoid allowing multiple selection on a select component.
  </CalloutText>
  <CalloutText>It might not be immediately clear to users that they need to press <inlineCode>Ctrl</inlineCode> or <inlineCode>Cmd</inlineCode> to choose multiple options on desktop, or that they <em>can</em> choose multiple options on mobile.</CalloutText>
  <CalloutText>Use a <inlineCode>CheckboxGroup</inlineCode> to allow users to make multiple selections instead.</CalloutText>
</Callout>
...
<LivePreview scope={{Select, Option, FormHelpText, FormLabel}} language="jsx">
  ...
  <FormHelpText variant="error" id="console_team_help_text">Please choose your team.</FormHelpText>
</>`}
</LivePreview>
...
<LivePreview scope={{Select, Option, FormHelpText, FormLabel}} language="jsx">
  ...
  <Select
    ...
    htmlFor="console_team"
  >
    <Option disabled value="messaging-console">Messaging Console</Option>
    <Option value="numbers-console">Numbers Console</Option>
    <Option disabled value="vnv-console">VNV Console</Option>
  </Select>
  <FormHelpText variant="default" id="console_team_help_text">Please choose your team.</FormHelpText>
</>`}
</LivePreview>
...
<LivePreview scope={{Select, Option, FormLabel, FormHelpText, Box}} language="jsx">
  {`<>
...
Use a safe and reversible option as the default selected value, or an option that tells users to choose from the list. For example, on page load, a select could show “Choose categories” as the default value if there’s no safe default.
...
Validating a form field when the user leaves the current field (on blur) can be helpful to check for required field entry. However, this can be frustrating to users who tab through controls to navigate a page, and to screen reader users, who might not be able to detect that an error occurred on blur.
...
<DoDont>
  <Do center title="Do" body="Sort options in a list logically (*e.g.*, alphabetically, ascending, descending).">
    <Box width="100%" padding="space60">
      <FormLabel htmlFor="sort_do">Which language do you use?</FormLabel>
...
<DoDont>
  ...
  >
    <Box width="100%" padding="space60">
      <FormLabel htmlFor="sort_do">Caller ID</FormLabel>
      <Select id="sort_do" name="sort_do" onChange="">
  ...
  >
    <Box width="100%" padding="space60">
      <FormLabel htmlFor="sort_do">Gender</FormLabel>
      <Select id="sort_do" name="sort_do" onChange="">
...
<DoDont>
  <Do
    center
    title="Do"
    body="Keep option text concise and simple. If choosing options require more explanation, consider using a checkbox instead and add help text for each option, or give more explanation through help text."
  />
  <Dont
    center
    title="Don't"
    body="Avoid long option text that will truncate if it’s longer than the width of the container."
  />
</DoDont>
...
<DoDont>
  ...
  <Dont
    center
    title="Don't"
    body="Avoid allowing multiple selection on a select component. It might not be immediately clear to users that they need to press `Ctrl` or `Cmd` to choose multiple options on desktop, or that they *can* choose multiple options on mobile."
  />
</DoDont>
...
<FormLabel htmlFor="foo" required>FooConsole team</FormLabel>
<Select aria-describedby="foo_text" id="foo" name="foo" type="text" placeholder="foo" onChange={FOO} required>
<Option value="foo-bar">Foo Bar</Option>
<Option value="bar-foo">Bar Foo</Option>
</Select>
<FormHelpText variant="default" id="foo_text">Please select one</FormHelpText>
```
