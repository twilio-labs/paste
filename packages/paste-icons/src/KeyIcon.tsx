...
export interface KeyIconProps extends IconWrapperProps {
  title?: string;
  decorative: boolean;
}

const KeyIcon = React.forwardRef<HTMLElement, KeyIconProps>(
  ({as, display, element = 'ICON', size, color, title, decorative}, ref) => {
    const titleId = `KeyIcon-${useUID()}`;

    if (!decorative && title == null) {
      throw new Error('[KeyIcon]: Missing a title for non-decorative icon.');
    }
