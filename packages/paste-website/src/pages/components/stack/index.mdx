...
### Justifying Stack content

Stacked elements can be aligned inside the Stack container using the `justifyContent` CSS flexbox property. This property
will only work if the Stack is set to `orientation="horizontal"`.

<LivePreview scope={{Anchor, Button, Stack}} language="jsx">
  {`<Stack orientation="horizontal" spacing="space60" justifyContent="flex-end">
  <Button variant="primary">Save</Button>
  <Button variant="secondary">Cancel</Button>
  <Anchor href="#">Close Project</Anchor>
</Stack>`}
...
| justifyContent | `ResponsiveValue<JustifyContentOptions>`    |                                                                                           | null    |
