...
export const pageQuery = graphql`
  {
    tokensDiagram: file(
      ...
      relativePath: {eq: "images/foundations/colors/tokens-diagram.png"}
    ) {
      childImageSharp {
        fluid(maxWidth: 800) {
    ...
    interactingStates: file(
      sourceInstanceName: {eq: "assets"}
      relativePath: {eq: "images/foundations/colors/interacting-states.png"}
    ) {
      childImageSharp {
        fluid(maxWidth: 800) {
          ...GatsbyImageSharpFluid
    ...
    useOfColorExample: file(
      sourceInstanceName: {eq: "assets"}
      relativePath: {eq: "images/foundations/colors/use-of-color-example.png"}
    ) {
      childImageSharp {
        fluid(maxWidth: 800) {
          ...GatsbyImageSharpFluid
    ...
    mdx(fields: {slug: {eq: "/foundations/colors/"}}) {
      fileAbsolutePath
      frontmatter {
        slug
        title
...
<Img fluid={props.data.tokensDiagram.childImageSharp.fluid} />
<TokensDiagramImgCaption />
...
A comprehensive list of all accessible color pairings is coming soon!
...
Dark Theme is in “early preview” and more work will be done to ensure color contrasts are accessible. Eventually, Dark Theme will be available for wider adoption, but this relies on full Paste adoption in a product. If there are parts of your UI that are not built with Paste, you should not adopt this early preview theme. If there are parts of the UI that aren’t using Paste, Dark theme wouldn’t be applied and the result will be a broken and inconsistent UI. For more questions, [join our discussion on GitHub](https://github.com/twilio-labs/paste/discussions/1527).
...
<Img fluid={props.data.useOfColorExample.childImageSharp.fluid} />
<UseOfColorImgCaption />
...
<Img fluid={props.data.interactingStates.childImageSharp.fluid} />
<InteractingStatesImgCaption />
