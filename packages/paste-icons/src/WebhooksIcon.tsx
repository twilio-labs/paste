...
export interface WebhooksIconProps extends IconWrapperProps {
  title?: string;
  decorative: boolean;
}

const WebhooksIcon: React.FC<WebhooksIconProps> = ({as, display, size, color, title, decorative}) => {
  const titleId = `WebhooksIcon-${useUID()}`;

  if (!decorative && title == null) {
    throw new Error('[WebhooksIcon]: Missing a title for non-decorative icon.');
  }
