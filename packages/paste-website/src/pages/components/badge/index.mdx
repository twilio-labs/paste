...
Following the [Paste Status Pattern](/patterns/status), Status Badges can be used in cases where a Badge represents either a Process or Connectivity. These composed Status Badges choose the right color and icon pairing for you, based on the variant you choose. The variants available are mapped to the [Status Pattern](/patterns/status).
