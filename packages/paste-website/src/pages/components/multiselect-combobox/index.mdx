...
Use an empty state to indicate to a user that their input doesn not match any value in the list of options.
...
Use a safe and reversible option as the default selected value, or an option that tells users to
choose from the list. For example, on page load, a select could show “Choose category" as the
default value if there is no safe default.
...
<DoDont>
  ...
  <Dont
    title="Don't"
    body="Don’t use a Multiselect Combobox when a user should only select a single option. Use a Combobox instead."
  />
</DoDont>
<DoDont>
  <Do
...
const Component = () => (
  <MultiselectCombobox items={foo} selectedItemsLabelText="Selected items:" labelText="Choose an item" />
);
```
...
| `i18nKeyboardControls?`  | `string`                                  | Visually hidden string that has instructions for how to remove and select pills with a keyboard.   |                                                                             |
