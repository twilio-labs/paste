...
<Callout variant="neutral" marginY="space70">
  <CalloutHeading as="h4">A note about Avatar contents</CalloutHeading>
  <CalloutText>
    The Avatar component can either display an image or an icon, but not both. If both are present, the Avatar will
    display the image.
  </CalloutText>
...
### Changing the shape of an Avatar

The Avatar `shape` prop can be used to change the shape. There are two shapes, one for users and one for entities, like a business. The default value for `shape` is "user".
...
>
  {`<Stack orientation="horizontal" spacing="space40">
  <Avatar shape="user" name="user example" icon={UserIcon} />
  <Avatar shape="entity" name="entity example" icon={BusinessIcon} />
</Stack>`}
</LivePreview>
...
| shape?   | `'user', 'entity'`                                                          | Specify the shape of the Avatar based on what it represents.                              | `'user'`     |
