...
The Product Switcher is an implementation of a Menu, that is designed to show the Twilio products a user has access to and can switch between.

It is designed to be used as a quick mechnism to jump between top level Twilio product applications such as Twilio Console, Segment, Sendgrid and Flex. It is ideally placed in the application [Topbar](/components/topbar).
...
The Product Switcher Button is a composed Icon Only Menu Button.

### Product Switcher

Container for the Product Switcher Menu.
...
### useProductSwitcherState state hook

React Hook used to initialise the Product Switcher with vaious options. It returns state and actions that can be taken on the Product Switcher menu.
