...
Grid column `span`, `offset`, and `gutter` props accept accept responsive values, allowing them to be changed on theme breakpoints.
For example, passing `span={[6, 8, 2] }` to a `Column`component sets the column `span` to be 6 columns wide on mobile, 8 columns wide on tablet, and 2 columns wide on desktop. The `offset`, `gutter`, and `vertical` props work in a similar fashion.
Check out the examples below and resize the browser to see the responsive values in action.

While you can achieve a one column responsive layout by changing each column `span` to 12, you lose out on the ability to have vertical spacing between each row. In this case, we recommend using the vertical prop as it will continue to apply the gutter spacing
...
<DoDont>
  <Do title="Do" body="Use a vertical grid to create a a stack columns.">
    <Box padding="space60">
      <Grid gutter="space30" vertical>
        <Column>
          <Card>
  ...
  <Dont title="Don't" body="Don’t use column span to stack columns.">
    <Box padding="space60">
      <Grid gutter="space30">
        <Column span={12}>
          <Card>
