...
- Set the `decorative` and `title` props correctly on your icons.
- Try to pair icons with text.
- Icons should not be interactive (i.e.: clickable or focuseable). They can be wrapped with an interactive element.
...
<LivePreview scope={{Button, Box}} language="jsx">
  ...
  <LoadingIcon decorative={true} size="sizeIcon30" color="colorText" />
</div>`}
</LivePreview>
...
set grows to contain over 100 icons, we don't want projects that only use a few icons to be burdened by
...
| title?     | string    | The accesibility text that is read when screenreaders get to this component.                                                                          |                |
