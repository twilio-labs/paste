...
<NormalizedComponentHeader
  ...
  storybookUrl="/?path=/story/components-tooltip--"
  data={props.data}
/>
...
A tooltip must be placed on **only a natively focusable HTML element**, like an enabled [Button](/components/button) or an [Anchor](/components/anchor). **Do not** place tooltips on non-focusable elements, like an icon, or disabled components, like a button. Keyboard users cannot focus on disabled components.
...
When the user focuses an element with a tooltip, their focus stays on the element. Since focus never goes inside the tooltip, do not use additional components, like an [Anchor](/components/anchor), within the Tooltip. 
...
Tooltips should contain only information that is useful additional context or guidance for the user. Furthermore, Tooltips are easily overlooked and disappear when not focused on, so consider whether the information may be better served by another component.

Never put information essential to task completion in a Tooltip. Use [Help Text](/components/help-text), which is persistent and more obvious, for essential information.

Do not use a Tooltip on a disabled [Button](/components/button) to explain why the button is unavailable. The disabled button isn't focusable. 

Similarly, do not including an [Anchor](/components/anchor) in Tooltip content because an Anchor isn't focusable. If you need to include a link, use a [Popover](/components/popover) instead.

Use full sentences and punctuation. Include no more than 3 lines of text. Avoid titles and icons.
...
<DoDont>
  <Do
    title="Do"
    body="Use Tooltips to provide additional helpful information or guidance."
    preview={false}
  />
  <Dont title="Don't" body="Don't use Tooltips for information essential to task completion." preview={false} />
</DoDont>

<DoDont>
  <Do title="Do" body="Use Tooltips only on focusable elements." preview={false} />
  <Dont
    title="Don't use Tooltips on non-focusable elements, like icons and disabled buttons."
    body="Don't use ."
    preview={false}
  />
</DoDont>
...
<DoDont>
  <Do
    title="Do"
    body="Include only text in Tooltip content."
    preview={false}
  />
  <Dont 
  title="Don't" 
  body="Don't include links in Tooltip content." 
  preview={false} 
  />
