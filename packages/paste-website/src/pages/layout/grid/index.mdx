...
<Callout>
  <CalloutTitle as="h4" variant="heading40">
    An important note about stacking columns.
  </CalloutTitle>
  <CalloutText>
    If you would like to achieve a one column responsive layout (or responsive stacked columns), we recommend using the{' '}
    <strong>vertical</strong>. You can also achieve this by setting each column `span` to 12, but the spacing between
    the stacked columns is not applied.
  </CalloutText>
</Callout>
