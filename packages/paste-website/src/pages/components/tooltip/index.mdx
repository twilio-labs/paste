...
A tooltip must be placed on **only a natively focusable HTML element**. Good candidates include a [Button](/components/button) or a [Anchor](/components/anchor) if the tip also links to a help article.

**Do not** place tooltips on non-focusable elements, like an icon or disabled buttons.
...
When the user focuses an element with a tooltip, their focus stays on the element. Since focus never goes inside the tooltip, avoid using additional components, like an [Anchor](/components/anchor), within the Tooltip. 
...
Similarly, avoid including an [Anchor](/components/anchor) in Tooltip content because the Anchor isn't focusable. If you need to include a link, use a [Popover](/components/popover) instead.
