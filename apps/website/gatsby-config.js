...
const gatsbyConfig = {
  ...
  plugins: [
    ...
    },
    {
      ...
      options: {
        typeName: ({node}) => {
          if (node.relativePath.startsWith('core/components') && node.relativePath.endsWith('package.json')) {
            return 'PasteComponent';
          }

          if (node.relativePath.startsWith('core/primitives') && node.relativePath.endsWith('package.json')) {
            return 'PastePrimitive';
          }

          if (node.relativePath.startsWith('core/layout') && node.relativePath.endsWith('package.json')) {
            return 'PasteLayout';
          }

          if (node.relativePath.startsWith('core/libraries') && node.relativePath.endsWith('package.json')) {
            return 'PasteLibraries';
          }

          if (node.relativePath.startsWith('core/theme') && node.relativePath.endsWith('package.json')) {
            return 'PasteThemePackage';
          }

          if (node.relativePath.startsWith('core/design-tokens') && node.relativePath.endsWith('package.json')) {
